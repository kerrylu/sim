...
describe('GET /api/table/[tableId]/rows', () => {
  ...
  it.each([
    ['filter', { filter: JSON.stringify({ conversation_id: { $eq: 'conversation-1' } }) }],
    ['sort', { sort: JSON.stringify({ updated_at: 'desc' }) }],
  ])('rejects Memory %s parameters before querying rows', async (_name, query) => {
    authAs('session')
    mockCheckAccess.mockResolvedValue({
      ok: true,
      table: {
    ...
    mockQueryRows.mockRejectedValue(
      new TableQueryValidationError('Filtering and sorting are not supported for this table')
    )
    ...
    expect(await res.json()).toEqual({
      error: 'Filtering and sorting are not supported for this table',
    })
